...
- **Only one person's week ever leaves the database.** The app calls one new Postgres function, `toolkit_my_week_hours(emp_id, date)`, which returns that employee's punches for that single week. The punches table is never downloaded to the device, and the tables don't need to be opened to the public key. Inactive employees and unknown IDs get a "no active employee found" message. The function also drops any punch that got double-loaded by a sync (same kind of protection as the v1.53.0 duplicate-pallet check).
- **Requires running `sql/my_hours_function.sql` once in the Supabase SQL Editor** before the screen will work.
- A shift that's still open (clocked in, not out yet) shows as "still clocked in" and counts toward the total once it's closed.

## v1.53.0
- **New: Data Health panel on Home.** Runs automatically on load and every 15 minutes — checks Inventory and Transactions directly against the raw Supabase tables (not the already-safe data the rest of the app shows), comparing row count to distinct pallet/LWH ID count. Green "Data looks healthy" when clean, red "Data issue detected" with the specific count if it finds a mismatch. Built directly out of the duplicate-pallet-count bug traced by hand over a full day in August 2026 — this is the same check that eventually solved it, just running automatically now instead of waiting for someone to notice a wrong number first. Requires two new Postgres functions (`get_inventory_health`, `get_transaction_health`) — see the accompanying SQL file.
- **New: Pallets by Location on Home.** A compact stat row (warehouse + pallet count) sitting right under the main KPI cards — the at-a-glance version of the existing detailed "Inventory by Location & Customer" table further down, which still has the per-customer breakdown for anyone who needs it. Computed client-side from the same already-loaded, already-hardened inventory data — no new network call needed for this one.
- **New: "Get Toolkit on Your Phone" QR card on Home.** Reuses the existing QR generator (same one Generate Code and Rack Labels already use) to show a scannable link straight to the live install URL, plus the URL as plain text/link underneath. About half the team runs this on desktop, other half mobile — makes it a one-scan install instead of typing a URL on a phone keyboard.
- A `get_pallets_by_location()` Postgres function is included in the SQL file too but not currently wired into the app — the Home widget above turned out simpler to build client-side from data already in memory. Left in as a spare in case it's useful for a future report.

## v1.52.3
- **Fixed: install icon showing a mismatched double-rounded edge.** The new icon (v1.52.2) already has its rounded corners and red background baked directly into the image, but `manifest.json` still declared it `purpose: "any maskable"` — that tells the OS it's free to apply its own reshaping on top, which collided with the shape already in the pixels and produced the odd seam/halo around the edge. Changed both icon entries to `purpose: "any"`, telling the OS to render the icon exactly as provided instead of reshaping it. Same fix belongs to the icon design working correctly, not a redesign — nothing about how the icon looks changed, just how the OS is told to handle it.
- Anyone who already reinstalled after v1.52.2 to test the new icon will likely need to remove and re-add it once more to pick up this fix, for the same OS-level install-icon-caching reason noted in that release.
...
- **Reordered shortcuts so Warehouse Tools ranks ahead of Bill of Lading** — in case a device/browser caps the visible shortcut list, Tools is now guaranteed a slot before BOL.
