(function(){
  ...
  function customerToPalletRow(r){ return {location:r.location||r.warehouse||'',lwhid:r.controlNumber||'',custId:r.comments||'',customer:r.subCustNm||'',invRec:r.invReceipt||'',billToRef:'',item:r.itemNm||'',desc:'',lot:r.lotNum||'',qty:r.qty||'',units:'',bay:r.currentBay||r.bayName||'',dateReceived:''}; }
  ...
  function printRows(list,target){ if(!list||!list.length){LWHUI.toast('No rows to print'); return;} LWHLabels.generatePalletRows(list,target||lookupPrintOutput); LWHStorage.set('lookupCount',(+LWHStorage.get('lookupCount',0))+1); }
