...
function palletDataFromBulk(){const rows=LWHUI.lines(palletBulkText.value).map(r=>r.split(/\t|,/).map(x=>x.trim()));if(rows[0]&&/lwh|customer|item/i.test(rows[0].join(' ')))rows.shift();return rows.map(r=>({lwhid:r[0],customer:r[1],item:r[2],lot:r[3],qty:r[4],bay:r[5],custId:r[6],date:r[7],desc:r[8]}))}
function generatePallet(){const out=palletOutput;out.innerHTML='';const rows=document.querySelector('[data-pallet-mode].active').dataset.palletMode==='bulk'?palletDataFromBulk():palletDataFromSimple();const copies=+palCopies.value||1;rows.filter(r=>Object.values(r).some(Boolean)).forEach(r=>{for(let i=0;i<copies;i++){const code=r.lwhid||r.bay||r.item||'PALLET';const qr=JSON.stringify(r);out.append(page('pallet-label',`<div class="pallet-head"><div><div class="pallet-title">${LWHUI.safe(r.customer||'PALLET LABEL')}</div><div>${LWHUI.safe(r.custId||'')}</div></div><div class="pallet-lwh">${LWHUI.safe(r.lwhid||'')}</div></div><div class="pallet-grid"><div><b>Item</b><br>${LWHUI.safe(r.item)}</div><div><b>Lot</b><br>${LWHUI.safe(r.lot)}</div><div><b>Qty</b><br>${LWHUI.safe(r.qty)}</div><div><b>Bay</b><br>${LWHUI.safe(r.bay)}</div><div><b>Date Received</b><br>${LWHUI.safe(r.date)}</div><div><b>Description</b><br>${LWHUI.safe(r.desc)}</div></div><div class="pallet-codes">${palBarcode.checked?`<div class="barcode-wrap"><svg data-barcode="${LWHUI.safe(code)}" data-height="75" data-width="2"></svg><div class="barcode-text">${LWHUI.safe(code)}</div></div>`:''}${palQR.checked?`<div class="qrbox" data-qr='${LWHUI.safe(qr)}' data-size="105"></div>`:''}</div>`))}});finishBarcodes(out);LWHStorage.set('printJobs',(+LWHStorage.get('printJobs',0))+out.children.length);LWHUI.toast(`Generated ${out.children.length} pallet label(s)`)}
function generateContact(){const out=contactOutput;out.innerHTML='';LWHUI.readFile(conLogo,logo=>{const c={name:conName.value,title:conTitle.value,company:conCompany.value,phone:conPhone.value,email:conEmail.value,website:conWebsite.value,address:conAddress.value};for(let i=0;i<(+conCopies.value||1);i++){out.append(page(`contact-card ${conLayout.value==='qronly'?'qronly':''}`,`<div class="qrbox" data-qr="${LWHUI.safe(LWHQR.vcard(c))}" data-size="122"></div>${conLayout.value==='qronly'?'':`<div class="contact-info">${logo?`<img class="contact-logo" src="${logo}">`:''}<div class="name">${LWHUI.safe(c.name)}</div><div>${LWHUI.safe(c.title)}</div><div><b>${LWHUI.safe(c.company)}</b></div><div>${LWHUI.safe(c.phone)}</div><div>${LWHUI.safe(c.email)}</div></div>`}`))}finishBarcodes(out);LWHStorage.set('printJobs',(+LWHStorage.get('printJobs',0))+out.children.length);LWHUI.toast(`Generated ${out.children.length} contact card(s)`)})}
window.LWHLabels={generateRack,generateSigns,generatePallet,generateContact,generateBatch};
