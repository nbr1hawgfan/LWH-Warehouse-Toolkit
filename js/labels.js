...
function palletLabelHtml(r){
  ...
  return `<div class="pallet-head"><div><div class="pallet-title">${safeAttr(r.customer||'PALLET LABEL')}</div></div><div class="pallet-lwh">${safeAttr(r.lwhid||'')}</div></div>${r.custId?`<div class="pallet-custid">${safeAttr(r.custId)}</div>`:''}<div class="pallet-info-row"><div class="pallet-grid"><div><b>Location</b><br>${safeAttr(r.location)}</div><div><b>Bay</b><br>${safeAttr(r.bay)}</div><div><b>Item</b><br>${safeAttr(r.item)}</div><div><b>Lot</b><br>${safeAttr(r.lot)}</div><div><b>Qty</b><br>${safeAttr(r.qty)}</div>${r.invRec?`<div><b>InvRec</b><br>${safeAttr(r.invRec)}</div>`:''}${r.billToRef?`<div><b>BillToRef</b><br>${safeAttr(r.billToRef)}</div>`:''}${r.vendor?`<div><b>Vendor</b><br>${safeAttr(r.vendor)}</div>`:''}${r.unique8?`<div><b>${(window.LWHInventory&&LWHInventory.customerLabels)?safeAttr(LWHInventory.customerLabels().unique8):'Item Description'}</b><br>${safeAttr(r.unique8)}</div>`:''}${r.date?`<div><b>Date Received</b><br>${safeAttr(r.date)}</div>`:''}${r.desc?`<div class="wide"><b>Description</b><br>${safeAttr(r.desc)}</div>`:''}</div>${custQr&&palCustQR.checked?`<div class="pallet-qr-side"><div class="qr-heading">CUSTOMER ID</div><div class="qrbox" data-qr="${safeAttr(custQr)}" data-size="90"></div><div class="cust-qr-value">${safeAttr(custQr)}</div></div>`:''}</div>${r.item?`<div class="pallet-item-big">${safeAttr(r.item)}</div>`:''}<div class="pallet-codes">${palBarcode.checked?`<div class="barcode-wrap lwh-code"><svg data-barcode="${safeAttr(code)}" data-height="55" data-width="2.3"></svg><div class="barcode-text">LWH ${safeAttr(code)}</div></div>`:''}</div>`;
}
