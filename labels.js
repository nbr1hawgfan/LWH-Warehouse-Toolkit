window.LWHUI={
  ...
  show(view){document.querySelectorAll('.view').forEach(v=>v.classList.toggle('active',v.id===view));document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view));const isHome=view==='home';const homeBtn=document.getElementById('homeNavBtn');if(homeBtn)homeBtn.hidden=isHome;const shell=document.querySelector('.app-shell');if(shell)shell.classList.toggle('home-active',isHome);window.scrollTo(0,0);if(window.LWHLabels&&LWHLabels.setPrintPageSize)LWHLabels.setPrintPageSize(null,null);if(view!=='utilities'&&window.LWHUtilities&&LWHUtilities.stopScannerCamera)LWHUtilities.stopScannerCamera();if(view!=='trailer'&&window.LWHTrailer&&LWHTrailer.stopLicenseCamera)LWHTrailer.stopLicenseCamera();},
